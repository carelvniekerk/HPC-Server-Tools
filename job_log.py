...
if __name__ == '__main__':
    ...
    if args.jobid is None:
        ...
        headers = ["Job number", "Job ID", "Name", "Job Status"]
        rows = [[job['job_num'], job['jobid'], job['name'], job['status']] for job in jobs]
    ...
    host = os.popen(f"qstat -f {jobid} -n | tail -n 1 | grep -o 'hilbert[0-9]*' | head -n1").read().strip()
